...
export class AgentLogger {
  ...
  logAgentThought(thought: string) {
    this.info('AGENT', `Agent thought: ${thought.substring(0, 300)}`);
  }

  logToolSelection(toolName: string, input: any) {
    this.info('TOOL_SELECT', `Selected tool: ${toolName}`, { input });
  }
  ...
  logRerankScores(scores: number[]) {
    this.info('RERANK', `Reranking scores: [${scores.map(s => s.toFixed(4)).join(', ')}]`);
  }
