...
function detectTechStack(files) {
    ...
    for (const file of files) {
        ...
        switch (ext) {
                ...
                if (file.path === 'package.json') {
                    try {
                        ...
                        if (pkg.dependencies) {
                            ...
                            if (deps.includes('nestjs') || deps.includes('@nestjs/core'))
                                stack.add('NestJS');
                            if (deps.includes('typeorm') || deps.includes('prisma') || deps.includes('sequelize'))
                                stack.add('ORM');
                            if (deps.includes('socket.io'))
                                stack.add('Socket.IO');
                        }
                        if (pkg.devDependencies) {
                            ...
                            if (devDeps.includes('typescript'))
                                stack.add('TypeScript');
                            if (devDeps.includes('jest') || devDeps.includes('vitest') || devDeps.includes('mocha'))
                                stack.add('Testing Framework');
                            if (devDeps.includes('tailwindcss') || devDeps.includes('postcss'))
                                stack.add('Tailwind CSS');
                            if (devDeps.includes('eslint'))
                                stack.add('ESLint');
                            if (devDeps.includes('prettier'))
                                stack.add('Prettier');
                            if (devDeps.includes('webpack') || devDeps.includes('vite') || devDeps.includes('rollup'))
                                stack.add('Bundler');
                        }
                    }
            ...
            case '.yaml':
            case '.yml':
                if (file.path === 'docker-compose.yml' || file.path === 'docker-compose.yaml')
                    stack.add('Docker');
                break;
            case '.go':
                stack.add('Go');
    ...
    if (allContent.includes('@app.route') || allContent.includes('flask'))
        stack.add('Flask');
    if (allContent.includes('from django') || allContent.includes('django.urls'))
        stack.add('Django');
    if (allContent.includes('require(\'express\')') || allContent.includes('from \'express\'') || allContent.includes('require("express")') || allContent.includes('from "express"'))
        stack.add('Express.js');
    if (allContent.includes('@Entity') || allContent.includes('@Table'))
        stack.add('TypeORM');
    if (allContent.includes('prisma'))
        stack.add('Prisma');
    return [...stack];
}
...
function getTotalLines(files) {
    return files.reduce((sum, f) => sum + f.content.split('\n').length, 0);
}
function generateProjectName(dirPath) {
    const name = path.basename(path.resolve(dirPath));
    return name;
}
async function processRepo(name, dirPath) {
    ...
    const files = (0, parser_1.scanDirectory)(resolvedPath);
    const chunks = (0, chunker_1.chunkFiles)(files);
