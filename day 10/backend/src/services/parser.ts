...
import { FileInfo, FunctionInfo, ClassInfo } from '../types';
...
function extractJSTS(content: string, lines: string[]): { functions: FunctionInfo[]; classes: ClassInfo[]; imports: string[]; exports: string[] } {
  ...
  while ((m = exportRegex.exec(content)) !== null) {
    const name = m[2];
    if (m[1]) exports.push(`default ${name}`);
...
function extractPython(content: string, lines: string[]): { functions: FunctionInfo[]; classes: ClassInfo[]; imports: string[]; exports: string[] } {
  const functions: FunctionInfo[] = [];
  const classes: ClassInfo[] = [];
  const imports: string[] = [];
  const exports: string[] = [];
...
function extractJava(content: string, lines: string[]): { functions: FunctionInfo[]; classes: ClassInfo[]; imports: string[]; exports: string[] } {
  const functions: FunctionInfo[] = [];
  const classes: ClassInfo[] = [];
  const imports: string[] = [];
  const exports: string[] = [];
...
function extractCPP(content: string, lines: string[]): { functions: FunctionInfo[]; classes: ClassInfo[]; imports: string[]; exports: string[] } {
  const functions: FunctionInfo[] = [];
  const classes: ClassInfo[] = [];
  const imports: string[] = [];
  const exports: string[] = [];
...
function findPythonBlockEnd(lines: string[], startLine: number): number {
  ...
  for (let i = startLine; i < lines.length; i++) {
    ...
    if (indent <= baseIndent && trimmed !== '') return i;
  }
  return lines.length;
}

function extractMethods(content: string, startLine: number, endLine: number): string[] {
  ...
  while ((m = methodRegex.exec(block)) !== null) {
    if (m[1] && m[1] !== 'class' && m[1] !== 'function') methods.push(m[1]);
  }
  return methods;
}

function extractPythonMethods(content: string, startLine: number, endLine: number): string[] {
  const methods: string[] = [];
  ...
  if (language === 'javascript' || language === 'typescript' || language === 'java' || language === 'cpp') {
    if (lines[0]?.trim().startsWith('/**') || lines[0]?.trim().startsWith('/*')) {
      ...
      for (const line of lines) {
        ...
        else if (line.trim().startsWith('/*')) commentLines.push(line.replace(/^\s*\/\*\*?\s?/, ''));
      }
      if (commentLines.length > 0) return commentLines.join(' ').trim().substring(0, 200);
    }
  ...
  if (language === 'python') {
    if ((lines[0]?.trim().startsWith('"""') || lines[0]?.trim().startsWith("'''"))) {
      ...
      for (let i = 1; i < lines.length; i++) {
        if (lines[i].trim().startsWith(quote)) break;
        commentLines.push(lines[i].trim());
      }
      if (commentLines.length > 0) return commentLines.join(' ').substring(0, 200);
    }
    if (lines[0]?.trim().startsWith('#')) {
      const commentLines: string[] = [];
      for (const line of lines) {
