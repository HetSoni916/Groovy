...
def interactive(provider):
    ...
    print(f"CLI chatbot v3 — {provider}  (logged to api_log.csv)")
    print("  /cost /usage /log  |  exit to quit")
    while True:
        try:
            u = input("> ")
        except EOFError:
        ...
        if u.strip() == "/usage":
            ...
            continue
        if u.strip() == "/log":
            print(f"  Log: {os.path.abspath('api_log.csv')}")
            continue
        ...
        log_call(provider, usage, elapsed, u, full)
...
def main():
    p = argparse.ArgumentParser(description="CLI chatbot v3 — token-tracking dashboard")
    ...
    if args.message:
        ...
        elapsed = time.time() - start
        log_call(args.provider, usage, elapsed, msg, text)
